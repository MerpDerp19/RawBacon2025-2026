...
public class FarAutoBlue extends LinearOpMode {
    ...
    private FarAutoBlue.LaunchState launchState;
    ...
    private FarAutoBlue.AutoState autoState;
...
*/
    ...
    public void runOpMode() throws InterruptedException {
        ...
        goToPosition(-24 * COUNTS_PER_INCH,48 * COUNTS_PER_INCH, 0.5, 14, 0.5 * COUNTS_PER_INCH);
        ...
        goToPosition(-6 * COUNTS_PER_INCH,0 * COUNTS_PER_INCH, 0.5, -7, 0.5 * COUNTS_PER_INCH);
        ...
        sleep(1000);
        goToPosition(-6 * COUNTS_PER_INCH, 6 * COUNTS_PER_INCH,0.5, -35, 0.5 * COUNTS_PER_INCH);
        launcher.setVelocity(250, AngleUnit.DEGREES);
        sleep(3000);
        leftFeeder.setPower(-1);
        rightFeeder.setPower(1);
        sleep(150);
        ...
        goToPosition(-24 * COUNTS_PER_INCH,6 * COUNTS_PER_INCH, 0.5, 0, 0.5 * COUNTS_PER_INCH);
...
//        GrabberPivot.setPosition(0.77);
    ...
    /*boolean launch(boolean shotRequested){
        switch (launchState) {
            case IDLE:
                if (shotRequested) {
                    launchState = MotifAutoBlue.LaunchState.PREPARE;
                    shotTimer.reset();
                    launcherTimer.reset();
                }
                break;
            case PREPARE:
                ...
                if (launcherTimer.seconds() > LAUNCH_TIME){
                    launchState = MotifAutoBlue.LaunchState.LAUNCH;
//                    leftFeeder.setPower(1);
//                    rightFeeder.setPower(1);
                    feederTimer.reset();
                }
            ...
            case LAUNCH:
                if (feederTimer.seconds() > FEED_TIME) {
                    ...
                    if(shotTimer.seconds() > TIME_BETWEEN_SHOTS){
                        launchState = MotifAutoBlue.LaunchState.IDLE;
                        return true;
                    }
                }
        }
